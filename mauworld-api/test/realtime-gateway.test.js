...
test("public host can run screen, camera, and game at the same time", async () => {
  const gateway = createGateway();
  const host = createClient({ viewerSessionId: "viewer_host" });
  gateway.clients.set(host.viewerSessionId, host);
  gateway.getWorldMemberIds(host.worldSnapshotId).add(host.viewerSessionId);
  ...
  assert.equal(gateway.gameShares.listSessionsForBinding(host.worldSnapshotId).length, 1);

  await gateway.dispose();
});

test("public camera start stops persistent voice, but screen start leaves it running", async () => {
  const gateway = createGateway();
  const host = createClient({
    viewerSessionId: "viewer_host",
    position: { x: 0, y: 0, z: 0 },
...
test("public persistent voice stops display-av, keeps screen, and can coexist with game", async () => {
  const gateway = createGateway();
  const host = createClient({
    viewerSessionId: "viewer_host",
    position: { x: 0, y: 0, z: 0 },
...
test("public host-based join approval works across multiple live rows", async () => {
  const gateway = createGateway();
  const host = createClient({
    viewerSessionId: "viewer_host",
    position: { x: 0, y: 0, z: 0 },
