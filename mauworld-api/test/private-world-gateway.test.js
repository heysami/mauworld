...
test("private game share inside an existing nearby game anchor returns join-required", async () => {
  const gateway = createGateway({
    async getWorldGame() {
      return {
        game: {
          id: "game_request",
          owner_profile_id: "profile_profile:requester",
          title: "Checkers",
  ...
  const anchorSession = gateway.gameShares.createSession({
    scope: "private",
    bindingKey: host.browserWorldKey,
  ...
  assert.equal(gateway.getHostedGameSession(requester.viewerSessionId, requester.browserWorldKey), null);

  await gateway.dispose();
});

test("approved private nearby game join creates an unlisted member game share", async () => {
  const gateway = createGateway({
    async getWorldGame() {
      return {
        game: {
  ...
  const memberSession = gateway.getHostedGameSession(requester.viewerSessionId, requester.browserWorldKey);
...
test("private origin can kick a nearby game contributor", async () => {
  const gateway = createGateway();
  const host = createClient({
    viewerSessionId: "profile:host",
    displayName: "host",
  ...
  const anchorSession = {
    ...
    binding_key: host.browserWorldKey,
    group_role: "origin",
    listed_live: true,
  };
  const memberSession = {
...
test("private origin can kick a nearby share contributor", async () => {
  const gateway = createGateway();
  const host = createClient({
    viewerSessionId: "profile:host",
    displayName: "host",
...
test("private persistent voice offer suppresses anchor listeners until approval", async () => {
  const gateway = createGateway();
  const anchorHost = createClient({
    viewerSessionId: "profile:anchor",
    displayName: "anchor",
    position: { x: 0, y: 0, z: 0 },
  });
  const anchorListener = createClient({
    viewerSessionId: "profile:listener",
  ...
  const outsider = createClient({
    viewerSessionId: "profile:outsider",
    displayName: "outsider",
    position: { x: 140, y: 0, z: 0 },
  });
  gateway.clients.add(anchorHost);
  gateway.clients.add(anchorListener);
  gateway.clients.add(voiceHost);
  gateway.clients.add(outsider);

  const anchorSession = {
    id: "anchor_session",
    sessionId: "anchor_session",
    hostSessionId: anchorHost.viewerSessionId,
    worldSnapshotId: anchorHost.browserWorldKey,
    sessionMode: "display-share",
    groupRole: "origin",
    listedLive: true,
  ...
  await gateway.rebalanceBrowserSessions(anchorHost.browserWorldKey);
