...
function buildScriptPrompt(input = {}) {
  ...
  return [
    ...
    "@run <every_tick|on_call|on_timer|on_overlap|on_hit|on_destroy>   // script.runtime only",
    ...
    "- use @target scene or @target <entity_id> for every_tick and event-driven scripts",
    "- use @run on_call for helper scripts that should only run when another script calls them",
    "- use @run on_timer plus @set timer_ms 1000 or @set interval_ms 1000 for recurring timer scripts",
    "- use @run on_overlap, @run on_hit, and @run on_destroy for event-driven scripts; inspect event.type and event.other / event.entity / event.cause",
    ...
    "- supported expressions: identifiers, property access, object literals, array literals, function calls, + - * /, comparisons, &&, ||, !",
    "- supported builtins: entity(id), entities(kind), players(), nearest(list, from), sort_by_distance(list, from), distance(a, b), normalize(v), length(v), vec(x,y,z), clamp(v,min,max), min(...), max(...), move_toward(from,to,max_step,stop_distance), spawn(source, position, rotation, options), destroy(entity), set_active(entity, bool), emit_particles(effect, at_or_on, options), raycast(origin, direction, max_distance, options), overlap(subject, options), find_by_group(group_id), call(function_id_or_name, ...args), play_sound(sound), stop_sound(sound)",
    "- special variables: self, scene, event, dt, time, plus any @set constants",
    "- return values: use return <expr>; call(...) returns the called script value",
    "- writable properties include self.position, self.position.x, self.rotation, self.velocity, self.active, scene.gravity, and visible/value/enabled where supported",
